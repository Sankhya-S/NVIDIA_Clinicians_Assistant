...
async def process_single_file(input_file_path, chat_wrapper, embedding_wrapper):
    ...
    noise_sensitivity_metric = NoiseSensitivity(llm=chat_wrapper)
    faithfulness_metric = Faithfulness(llm=chat_wrapper)
    # Evaluate scores
    relevancy_scores, context_precision_scores, noise_sensitivity_scores, faithfulness_scores = await evaluate_scores(data_dict, context_precision_metric, relevancy_metric, noise_sensitivity_metric, faithfulness_metric)
    
    # Update data dictionary with scores
    for i, (rel_score, cp_score, noise_score, faith_score) in enumerate(zip(relevancy_scores, context_precision_scores, noise_sensitivity_scores, faithfulness_scores)):
        data_dict[i]['context_precision_score'] = cp_score
        data_dict[i]['relevancy_score'] = rel_score
        data_dict[i]['noise_sensitivity_score'] = noise_score
        data_dict[i]['faithfulness_score'] = faith_score
...
async def evaluate_scores(dataset, context_precision_metric, relevancy_metric, noise_sensitivity_metric, faithfulness_metric):
    ...
    for idx, entry in enumerate(dataset):
        ...
        try:
            ...
            context_precision_score, relevancy_score, noise_sensitivity_score, faithfulness_score = await asyncio.gather(
                context_precision_metric.single_turn_ascore(sample1),
                relevancy_metric.single_turn_ascore(sample2),
                noise_sensitivity_metric.single_turn_ascore(sample3),
                faithfulness_metric.single_turn_ascore(sample3)
            )
        ...
        noise_sensitivity_scores.append(noise_sensitivity_score)
        print("Successfully processed entry")
    
    return relevancy_scores, context_precision_scores, noise_sensitivity_scores, faithfulness_scores
