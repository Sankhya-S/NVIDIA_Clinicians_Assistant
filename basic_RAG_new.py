...
"""
    ...
    def process_questions(self, questions_csv: str, output_csv: str, chunk_size: int = 10):
        ...
        try:
            ...
            questions_df['source_metadata'] = None
            questions_df['temporal_analysis'] = None
            questions_df['source_type_analysis'] = None
            
            for idx, row in questions_df.iterrows():
                try:
                    response = self.qa_chain(row['user_input'])
                    
                    questions_df.at[idx, 'answer'] = response['result']
                    questions_df.at[idx, 'source_content'] = str([
                        doc.page_content[:200] for doc in response['source_documents']
                    ])
                    questions_df.at[idx, 'source_metadata'] = str(response['metadata_analysis'])
                    questions_df.at[idx, 'temporal_analysis'] = str(response['temporal_range'])
                    questions_df.at[idx, 'source_type_analysis'] = str(response['source_types'])
